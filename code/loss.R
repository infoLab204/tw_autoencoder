...
for(i in 1:nrow(PCA_recon)){
    ...
    VAE_recon_loss[i] <- mean((X_data[i, ] - VAE_recon[i, ])^2)
}
  
total_rec_loss <- data.frame(model_type = rep(c("LAE", "SAE", "PCA", "ICA"), each = length(PCA_recon_loss)), loss = c(LAE_recon_loss, SAE_recon_loss, PCA_recon_loss, ICA_recon_loss,VAE_recon_loss))
...
for(class_index in 0:9) {  
  ...
  VAE_recon_class <- read.csv(paste0(data_type, "_VAE_recon_", code, "_class", class_index, ".csv"), header = F)  
  ...
  for(i in 1:nrow(PCA_recon_class)){
      ...
      VAE_recon_class_loss[i] <- mean((X_data[i, ] - VAE_recon_class[i, ])^2)
  }
    
  total_recon_loss <- data.frame(model_type = rep(c("LAE", "SAE", "PCA", "ICA","VAE"), each = length(PCA_recon_class_loss)), class_index = class_index, loss = c(LAE_recon_class_loss,SAE_recon_class_loss, PCA_recon_class_loss, ICA_recon_class_loss,VAE_recon_class_loss))
